...
import { Box, Typography, Paper, CircularProgress, Alert, Avatar, Stack } from '@mui/material';
...
function useStatsData() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      axios.get(`${API_URL}/posts/stats/new-per-month`, { headers }),
      ...
      .then(([postsRes, usersRes, topUsersRes, byDayRes, byHourRes, topGroupsRes]) => {
    ...
    <Box sx={{ width: '100%', height: 180 }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 30 }}>
          ...
          <XAxis dataKey="label" fontSize={11} angle={-30} textAnchor="end" height={50} />
          <YAxis allowDecimals={false} fontSize={12} />
          <Tooltip />
          <Bar dataKey="count" fill="#ffb6d5" radius={[6, 6, 0, 0]} />
        </BarChart>
      </ResponsiveContainer>
      <Typography align="center" fontWeight={700} fontSize={15} color="#dc004e" mt={1}>
        New Posts Per Month
      </Typography>
    </Box>
  );
    ...
    <Box sx={{ width: '100%', height: compact ? 160 : 220 }}>
      <ResponsiveContainer width="100%" height="100%">
        <BarChart
          data={data}
          layout="vertical"
          margin={compact ? { top: 10, right: 10, left: 30, bottom: 10 } : { top: 20, right: 20, left: 40, bottom: 20 }}
          barSize={compact ? 18 : 28}
        >
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis type="number" allowDecimals={false} fontSize={compact ? 11 : 12} />
          <YAxis dataKey="username" type="category" fontSize={compact ? 11 : 13} width={compact ? 70 : 100}
            tick={({ x, y, payload }) => {
              const user = data.find(u => u.username === payload.value);
              const img = getProfilePictureUrl(user?.profilePicture);
              return (
                <g transform={`translate(${x - (compact ? 18 : 30)},${y + (compact ? 8 : 10)})`}>
                  <foreignObject width="24" height="24">
                    <Avatar src={img} sx={{ width: 20, height: 20, mr: 1, display: 'inline-block', verticalAlign: 'middle' }} />
                  </foreignObject>
                  <text x={compact ? 26 : 35} y={0} dy={0} fontSize={compact ? 11 : 13} fill="#222">{payload.value}</text>
                </g>
              );
            }}
          />
      ...
      <Typography align="center" fontWeight={700} fontSize={15} color="#dc004e" mt={1.5} mb={compact ? 0.5 : 2}>
    ...
    <Box sx={{ width: '100%', height: 180 }}>
      <ResponsiveContainer width="100%" height="100%">
        <LineChart data={data} margin={{ top: 20, right: 10, left: 0, bottom: 30 }}>
          ...
          <YAxis allowDecimals={false} fontSize={12} />
          <Tooltip />
          <Line type="monotone" dataKey="count" stroke="#ffb6d5" strokeWidth={3} dot={{ r: 5 }} />
        </LineChart>
      </ResponsiveContainer>
      <Typography align="center" fontWeight={700} fontSize={15} color="#ffb6d5" mt={1}>
        Posts by Hour
      </Typography>
    </Box>
  );
...
export default function StatisticsGraphs({ compact }) {
  ...
  return (
    <Paper elevation={2} sx={{ p: 3, mb: 4, borderRadius: 3 }}>
      <Typography
        variant="h5"
        sx={{ mb: 2, fontWeight: 700, color: '#222', textAlign: 'center', cursor: 'pointer', textDecoration: 'underline' }}
        component={Link}
        to="/statistics"
      >
        Statistics
